...
{
    public class EzDataManagerWriter
    {
        const string ValidChars = "abcdefghdijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_";
...
";
        ...
        #region Variable PlayerPrefs Save/Load
        #region Save/Load Category
        const string saveCategHeader =
@"
        public static void SaveCategoryToPlayerPrefs(string categoryName)
        {
            switch(categoryName)
            {
";
        const string saveCategFooter =
@"                default:
                    Debug.Log(""[EzDataManager] Category "" + categoryName + "" not found! Nothing to save to PlayerPrefs"");
                    return;
            }
            PlayerPrefs.Save();
        }";
        const string loadCategHeader =
@"
        public static void LoadCategoryFromPlayerPrefs(string categoryName)
        {
            switch(categoryName)
            {
";
        const string loadCategFooter =
@"                default:
                    Debug.Log(""[EzDataManager] Category "" + categoryName + "" not found! Nothing to load from PlayerPrefs"");
                    break;
            }
        }";
        #endregion
        #region Save/Load All
        const string autoSaveAllMethod =
@"
...
";
        const string saveAllHeader =
@"
        public static void SaveAllToPlayerPrefs()
        {
";
        const string saveAllFooter =
@"            PlayerPrefs.Save();
        }";
        const string autoLoadAllMethod =
@"
        partial void AutomaticLoadAll()
        {
            LoadAllFromPlayerPrefs();
        }
";
        const string loadAllHeader =
@"
        public static void LoadAllFromPlayerPrefs()
        {
";
        const string loadAllFooter =
@"        }";
        #endregion
        #endregion //Variable PlayerPrefs Save/Load
...
#pragma warning restore 0649
        ...
        public static void GenerateEzDataManagerScript(Dictionary<string, List<EzDataManagerEditor.Variable>> data)
        {
            ...
            var saveCategBuilder = new StringBuilder(saveCategHeader);
            var loadCategBuilder = new StringBuilder(loadCategHeader);

            var saveAllBuilder = new StringBuilder(saveAllHeader);
            var loadAllBuilder = new StringBuilder(loadAllHeader);
            ...
            if(data != null && data.Keys.Count > 0)
            {
                ...
                foreach(var key in data.Keys)
                {
                    ...
                    numberOfItems = 0;
                    saveCategBuilder.AppendLine(@"                case """ + key + @""":");
                    loadCategBuilder.AppendLine(@"                case """ + key + @""":");
                    if(data[key] != null && data[key].Count > 0)
                    {
                        foreach(var v in data[key])
                        {
                            ...
                            {
                                // functions for PlayMaker support
                                switch(v.typeName)
                                {
                                    case "int":
                            ...
                            if(v.saveVariable)
                            {
                                saveCategBuilder.AppendLine(GetSaveToPrefsLine(v.name, true, v.useEncryption, v.encryptionKey));
                                loadCategBuilder.AppendLine(GetLoadFromPrefsLine(v.name, true, v.useEncryption, v.encryptionKey));
                                saveAllBuilder.AppendLine(GetSaveToPrefsLine(v.name, false, v.useEncryption, v.encryptionKey));
                                loadAllBuilder.AppendLine(GetLoadFromPrefsLine(v.name, false, v.useEncryption, v.encryptionKey));
                            }
                        }
                    }
                    saveCategBuilder.AppendLine("                    break;");
                    loadCategBuilder.AppendLine("                    break;");
                    sb.Append(GetEndCategory(key, categoryId, numberOfItems));
                    sb.AppendLine();
                    categoryId++;
                }
            }
            sb.Append("        #region Getter/Setter Functions");
            ...
            // PlayerPrefs Helpers
            sb.AppendLine();
            sb.Append("        #region PlayerPrefs Helpers");
            sb.Append(saveCategBuilder.AppendLine(saveCategFooter));
            sb.Append(loadCategBuilder.AppendLine(loadCategFooter));
            sb.Append(autoSaveAllMethod);
            sb.Append(saveAllBuilder.AppendLine(saveAllFooter));
            sb.Append(autoLoadAllMethod);
            sb.Append(loadAllBuilder.AppendLine(loadAllFooter));
            sb.AppendLine("        #endregion //PlayerPrefs Helpers");

            sb.Append(sDataManagerFooter);

            Debug.Log("[EzDataUtility] Generating EzDataManagerVars.cs");
            string filePath = EZT.RELATIVE_PATH_DATA_MANAGER + "Scripts/" + "EzDataManagerVars.cs";
        ...
        static string GetSaveToPrefsLine(string varName, bool addExtraTab, bool encrypted, int keyIndex)
        {
            StringBuilder sb = new StringBuilder();
            if(addExtraTab) { sb.Append("        "); }
            sb.Append("            EzDataManager.Instance.");
            sb.Append(varName);
            if(encrypted) { sb.Append(".Encrypt("); sb.Append(keyIndex); sb.Append(")"); }
            sb.Append(@".Save(""");
            sb.Append(varName);
            sb.Append(@"""); ");
            return sb.ToString();
        ...
        static string GetLoadFromPrefsLine(string varName, bool addExtraTab, bool encrypted, int keyIndex)
        {
            StringBuilder sb = new StringBuilder();
            if(addExtraTab) { sb.Append("        "); }
            sb.Append("            EzDataManager.Instance.");
            ...
            sb.Append(varName);
            sb.Append(@"""");
            if(encrypted) { sb.Append(", true, "); sb.Append(keyIndex); }
            sb.Append(");");
            return sb.ToString();
        }
